...
async function getTestGroups(repoDirPath, onlyChanged = false) {
  let libDefs = await (0, _libDefs.getLibDefs)(repoDirPath);

  if (onlyChanged) {
    const diff = await (0, _git.getDiff)();
    let changedDefs; // $FlowFixMe
...
async function writeFlowConfig(repoDirPath, testDirPath, libDefPath, version) {
  ...
  const flowConfigData = ['[libs]', _node.path.basename(libDefPath), _node.path.join(repoDirPath, '..', '__util__', 'tdd_framework.js'), '', '[options]', 'suppress_comment=\\\\(.\\\\|\\n\\\\)*\\\\$ExpectError', 'include_warnings=true', 'server.max_workers=0', '', // Be sure to ignore stuff in the node_modules directory of the flow-typed
  // CLI repository!
...
async function runTestGroup(repoDirPath, testGroup, orderedFlowVersions) {
  ...
  try {
    ...
    await writeFlowConfig(repoDirPath, testDirPath, testGroup.libDefPath, lowestFlowVersionRan);
    const flowErrors = await runFlowTypeDefTests(flowVersionsToRun, testGroup.id, testDirPath);
...
async function runTests(repoDirPath, testPatterns, onlyChanged, numberOfFlowVersions) {
...
async function run(argv) {
  ...
  let repoDirPath = (await _node.fs.exists(cwdDefsNPMPath)) ? cwdDefsNPMPath : _node.path.join(__dirname, '..', '..', '..', 'definitions', 'npm');
  ...
  try {
    results = await runTests(repoDirPath, testPatterns, onlyChanged, numberOfFlowVersions);
  } catch (e) {
