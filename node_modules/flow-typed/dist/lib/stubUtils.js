...
async function resolvePkgDirPath(pkgName, pkgJsonDirPath) {
...
async function writeStub(projectRoot, packageName, packageVersion, overwrite, files, libdefDir, maxDepth, typescriptTypingsPath, typescriptTypingsContent) {
...
`;
  ...
  if (!overwrite) {
    ...
    if (exists) {
      const existingStub = await _node.fs.readFile(filename);
...
async function pkgHasFlowFiles(projectRoot, packageName) {
...
async function createStub(projectRoot, packageName, explicitVersion, overwrite, typescript, libdefDir, maxDepth) {
  let files = [];
  ...
  try {
    ...
    const filename = await writeStub(projectRoot, packageName, version, overwrite, files, typedefDir, maxDepth, typescriptTypingsPath, typescriptTypingsContent);
